...
- **멀티모달(Vision) 통합:** LLaVA나 Qwen-VL을 도입해, 보호자가 텍스트뿐 아니라 반려견 피부 상태 사진 같은 이미지를 올리면 이를 분석해 RAG 지식과 결합한 조언을 제공하는 기능을 추가할 예정입니다.
- **개인화 프로필 DB:** C++ 내장 SQLite를 연동해 반려견의 이름·나이·견종·기저질환을 영구 저장하고, 이를 시스템 프롬프트에 자동 주입하는 방식으로 더 맞춤화된 답변을 제공할 계획입니다.
